    ...
    var WeeklyTimesheet = core.form_custom_registry.get('weekly_timesheet').extend({
        ...
        initialize_content: function() {
            ...
            return self.render_drop.add(new Model('account.analytic.line').call('default_get', [
                ['account_id','general_account_id','journal_id','date','name','user_id','product_id','product_uom_id','amount','unit_amount','is_timesheet','task_id', 'required_task_id'],
                    ...
                    var days = _.map(dates, function(date) {
                        ...
                        if (to_add) {
                            ...
                            day.lines.unshift(_.extend(_.clone(account_defaults), {
                                ...
                                required_task_id:lines[0].required_task_id,
                            }));
                        }
                        return day;
                    });
        ...
        init_add_account: function() {
            ...
            self.task_required = new FieldBool(self.dfm, {
                attrs: {
                    options: '{"create": False, "create_edit": False, "no_create": True}',
            ...
            self.task_required.prependTo(this.$('.o_add_timesheet_line > div')).then(function() {
                self.task_required.$el.addClass('o_form_invisible');
            });

            self.account_m2o.$input.on('autocompleteclose', function(){
            ...
            self.$('.oe_timesheet_button_add').click(function() {
                ...
                var task_required = self.task_required.get_value();
                if (id === false || (task_required === true & task_id === false)) {
                ...
                ops.push(_.extend({}, self.default_get, {
                    ...
                    account_id: id,
                    task_id: task_id,
                    required_task_id: task_required
                }));
        ...
        onchange_account_id: function() {
            ...
            new Model('account.analytic.account').call("read",
                [account_id,['required_task_id']],  {'context': {'default_is_timesheet':true}})
            .then(function(result) {
                self.task_m2o
                if (result['required_task_id'] == true){
                    self.task_m2o.node.attrs.modifiers = '{"required": true}';
                    self.task_required.set_value(true);
                    self.task_required.render_value();
                    self.task_m2o.modifiers.required = true;
                    self.task_m2o.$el.addClass('o_form_required');
                } else {
                    self.task_m2o.node.attrs.modifiers = '{"required": false}';
                    self.task_required.set_value(false);
                    self.task_required.render_value();
                    self.task_m2o.modifiers.required = false;
                    self.task_m2o.$el.removeClass('o_form_required');
                }
